...
class Sparse_GP: 
    ...
    def compute_output(self):
        ...
        self.KzzInv = T.nlinalg.matrix_inverse(self.Kzz)
        ...
        self.covCavity = T.nlinalg.matrix_inverse(self.covCavityInv)
        ...
        self.covPosterior = T.nlinalg.matrix_inverse(self.covPosteriorInv)
    ...
    #
    def log_det_psd(slef,A):
        L = T.slinalg.cholesky(A)
        return 2 * T.sum(T.log(T.diag(L)))
    def getLogNormalizerCavity(self):
        ...
        return casting(0.5 * self.n_inducing_points * np.log(2 * np.pi)) + casting(0.5) * self.log_det_psd(self.covCavity) + \
    ...
    def getLogNormalizerPrior(self):
        ...
        return casting(0.5 * self.n_inducing_points * np.log(2 * np.pi)) - casting(0.5) * self.log_det_psd(self.KzzInv)
    ...
    def getLogNormalizerPosterior(self):
        ...
        return casting(0.5 * self.n_inducing_points * np.log(2 * np.pi)) + casting(0.5) * self.log_det_psd(self.covPosterior) + \
