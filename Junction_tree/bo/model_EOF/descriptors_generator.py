...
def CDS_descriptors(count, smi):
    ...
    try:
        ...
        AllChem.UFFOptimizeMolecule(mol2, maxIters = 2000, ignoreInterfragInteractions = True)
        AllChem.MMFFOptimizeMolecule(mol2, maxIters = 2000, mmffVariant='MMFF94s', ignoreInterfragInteractions = True)
    ...
    patt_OCH3 = Chem.MolFromSmarts('[OX2](-[#6])-[CH3]')
    d_n_oc = _CountMatches(mol2, patt_OCH3, unique=True) #substituion for function fr_OCH3
  ...
  #  d_4 = Chem.Fragments.fr_nitro(mol)
    patt_NH2 = Chem.MolFromSmarts('[NH2,nH2]')
    d_5 = _CountMatches(mol2, patt_NH2, unique=True) #substituion for function fr_NH2
  ...
  #  d_mol.append(d_21)
   ...
   # print(d_mol)
    ...
    formal_charge = Chem.GetFormalCharge(mol2)
    return d_mol, formal_charge
    
def generator(smiles, fragment = False, mode = None):
    # if 'dataset' in input_filename:
    #     dataset = pd.read_excel(input_filename)
    ...
    if fragment:
        if mode == 'cat':
            list_all = np.zeros((len(smiles),total_len*2))
        else :
    ...
    for count, smi in enumerate(smiles):
       ...
       # print(d_mol_new)
        if fragment:
            ...
            #print(smi1)
            #print(smi2)
            d_mol_1, formal_charge1 = CDS_descriptors(count, smi1)
            d_mol_2, formal_charge2 = CDS_descriptors(count, smi2)
            #print(formal_charge1, formal_charge2)
            
                
            if d_mol_1 and d_mol_2:
                
                
                    
                if mode == 'cat':
                    
                    if (formal_charge1 != 1 and formal_charge2 != -1):
                        print('Exchange sequence of anion and cation')
                        d_mol_1, d_mol_2 = d_mol_2, d_mol_1
                        
                    list_all[count] = d_mol_1+d_mol_2
                else:
                    list_all[count] = np.sum([d_mol_1,d_mol_2], axis = 0)
            else:
                continue
                
        else:
            
            d_mol, formal_charge = CDS_descriptors(count, smi)
            if d_mol:
    ...
    if fragment:
        if mode == 'cat':
            ...
            composite_descriptors = composite_descriptors + composite_descriptors
        else:
            composite_descriptors = descriptors_name + ['ESTATE_'+ str(i) for i in range(E_state_len)]
    else:
        composite_descriptors = descriptors_name + ['ESTATE_'+ str(i) for i in range(E_state_len)]
...
# if __name__ == '__main__':
    # print('Running the code itself\n--------------------------')
    # A = generator(['CC'],fragment = False)