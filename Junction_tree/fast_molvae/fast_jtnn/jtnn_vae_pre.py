...
class JTNNVAE_PRE(nn.Module):
    ...
    def __init__(self, vocab, hidden_size, latent_size, depthT, depthG):
        ...
        self.tree_vecs_lin = nn.Sequential(
            nn.Linear(hidden_size, hidden_size), 
            nn.ReLU(),         
            nn.Linear(hidden_size, hidden_size), 
            nn.ReLU(),  
        ...
        self.tree_mess_lin = nn.Sequential(
            nn.Linear(hidden_size, hidden_size), 
            nn.ReLU(),         
            nn.Linear(hidden_size, hidden_size), 
            nn.ReLU(),  
        ...
        self.mol_vecs_lin = nn.Sequential(
            nn.Linear(hidden_size, hidden_size), 
            nn.ReLU(),         
            nn.Linear(hidden_size, hidden_size), 
            nn.ReLU(),  
        ...
        '''
        self.tree_vecs_lin_1 = nn.Linear(hidden_size, hidden_size)
        self.tree_mess_lin_1 = nn.Linear(hidden_size, hidden_size)
        self.mol_vecs_lin_1 = nn.Linear(hidden_size, hidden_size)
        
        self.tree_vecs_lin_2 = nn.Linear(hidden_size, hidden_size)
        self.tree_mess_lin_2 = nn.Linear(hidden_size, hidden_size)
        self.mol_vecs_lin_2 = nn.Linear(hidden_size, hidden_size)
        '''
        
    def load_model(self):
    
        vocab_path = '/home/shuoxing/wrh/Junction_tree/icml18-jtnn-master/data/moses/vocab.txt'
        vocab = [x.strip("\r\n ") for x in open(vocab_path)] 
        vocab = Vocab(vocab)
        model_pre_path='/home/shuoxing/wrh/Junction_tree/icml18-jtnn-master/fast_molvae/moses-h450z56/model.iter-400000'
        model_pre = JTNNVAE(vocab, 450, 56, 20, 3)
        model_pre.load_state_dict(torch.load(model_pre_path))
    ...
    def encode(self, jtenc_holder, mpn_holder):
       ...
       tree_vecs = self.tree_vecs_lin(tree_vecs)
       
       tree_mess = self.tree_mess_lin(tree_mess)
       
       mol_vecs = self.mol_vecs_lin(mol_vecs)
    ...
    def encode_from_smiles_mean(self, smiles_list):
        tree_batch = [MolTree(s) for s in smiles_list]
        _, jtenc_holder, mpn_holder = tensorize(tree_batch, self.vocab, assm=False)
        mean,var = self.encode_latent(jtenc_holder,mpn_holder)
        return mean,var
