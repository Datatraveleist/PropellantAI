...
class JTNNVAE(nn.Module):
    ...
    #     tree_batch = [MolTree(smiles) for s in smiles]
    #     _, jtenc_holder, mpn_holder = tensorize(tree_batch, self.vocab, assm=False)
    ...
    def reconstruct(self, smiles_list, prob_decode=False):
        if isinstance(smiles_list, str):
            smiles_list = [smiles_list]  # 单个字符串也封装成列表
        ...
        for i in range(len(smiles_list)):
            eps_tree = create_var(torch.randn(1, self.latent_size), False)
            eps_mol = create_var(torch.randn(1, self.latent_size), False)

            # vec_tree = tree_mean[i:i+1] + torch.exp(tree_log_var[i:i+1] / 2) * eps_tree
            # vec_mol = mol_mean[i:i+1] + torch.exp(mol_log_var[i:i+1] / 2) * eps_mol
            vec_tree = tree_mean[i:i+1] 
            vec_mol = mol_mean[i:i+1] 

            try:
                smiles = self.decode(vec_tree, vec_mol, prob_decode)
            except:
                smiles = None
            decoded_smiles.append(smiles)

        return decoded_smiles if len(decoded_smiles) > 1 else decoded_smiles[0]
    ...
    def encode_latent_mean(self, smiles_list):
        ...
        _, tree_vec, mol_vec = self.encode_mols(mol_batch)
        tree_mean = self.T_mean(tree_vec)
        mol_mean = self.G_mean(mol_vec)
        return torch.cat([tree_mean,mol_mean], dim=1)
    def encode_mols(self, mol_batch):
