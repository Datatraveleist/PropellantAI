...
parser.add_argument('--train', type=str,default = 'fdp-processed-git')
parser.add_argument('--vocab', type=str,default = 'vocab.txt')
parser.add_argument('--save_dir', type=str,default = 'vae_fdp')
...
parser.add_argument('--anneal_iter', type=int, default=7000)
parser.add_argument('--kl_anneal_iter', type=int, default=400)
...
for epoch in range(args.epoch):
    loader = MolTreeFolder(args.train, vocab, args.batch_size, num_workers=24)
    #print('***************123****************')
    for batch in loader:
        total_step += 1
        #print(total_step)
        ...
        if total_step % args.save_iter == 0:
            torch.save(model.state_dict(), args.save_dir + "/model_fdp.iter-" + str(total_step))
            # break
